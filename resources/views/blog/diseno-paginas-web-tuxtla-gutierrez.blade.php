@php
    $bp = trans('blog.tuxtla_web_design_page');
    $canonical = url('/blog/diseno-paginas-web-tuxtla-gutierrez');
    $htmlLang = 'es-MX';
    $landingUrl = route('landing.webdesign.tuxtla.chiapas');
    $seo_overrides = [
        'title' => $bp['seo_title'],
...
<div class="relative pt-28 sm:pt-32 pb-20 px-4 sm:px-6 overflow-hidden">
    ...
    <div class="relative max-w-4xl mx-auto">
        <nav aria-label="{{ __('seo.breadcrumb.home') }}" class="mb-8 text-sm text-gray-500">
            <ol class="flex flex-wrap items-center gap-x-2 gap-y-1">
                <li><a href="{{ route('home') }}" class="hover:text-black transition-colors">{{ __('seo.breadcrumb.home') }}</a></li>
                <li aria-hidden="true">/</li>
                <li><a href="{{ route('blog.index') }}" class="hover:text-black transition-colors">{{ __('seo.breadcrumb.blog') }}</a></li>
                <li aria-hidden="true">/</li>
                <li><span aria-current="page" class="text-gray-800 font-medium">Diseño web Tuxtla</span></li>
            </ol>
        </nav>
        ...
        <header class="text-center">
            ...
            <div class="mt-10 flex flex-col sm:flex-row flex-wrap items-center justify-center gap-3">
                <a href="{{ $landingUrl }}"
                   ...
                   data-track="blog_tuxtla_web_cta_landing">
                    {{ $bp['hero_cta_landing'] }}
                </a>
                <a href="{{ route('website_quote') }}"
                   ...
                   data-track="blog_tuxtla_web_cta_quote">
                    {{ $bp['hero_cta_quote'] }}
                </a>
                <a href="{{ route('contact') }}"
                   ...
                   data-track="blog_tuxtla_web_cta_contact">
                    {{ $bp['hero_cta_contact'] }}
                </a>
            </div>
        </header>
            ...
            <div class="mt-5 grid sm:grid-cols-2 gap-3">
                @foreach ($bp['toc'] as $item)
                    <a href="#{{ $item['id'] }}"
                       ...
                       data-track="blog_tuxtla_web_toc_{{ $item['id'] }}">
        ...
        @foreach ($bp['sections'] as $section)
            <section id="{{ $section['id'] }}" class="mt-16 scroll-mt-28">
                ...
                @if ($section['id'] === 'con-clics')
                    <p class="mt-6">
                        <a href="{{ route('website_quote') }}" class="font-semibold text-black underline hover:no-underline" data-track="blog_tuxtla_web_inline_quote">
                            Abrir cotizador por clics →
                        </a>
                    </p>
                @endif
            </section>
        ...
        <section class="mt-16 rounded-2xl border border-gray-200 bg-white p-6 sm:p-8 shadow-sm" aria-labelledby="blog-tuxtla-faq-title">
            @php
                $faqSchema = [
                    '@context' => 'https://schema.org',
                    '@type' => 'FAQPage',
            ...
            <h2 id="blog-tuxtla-faq-title" class="text-2xl sm:text-3xl font-bold text-black">{{ $bp['faq_title'] }}</h2>
        ...
        <section class="mt-16 rounded-2xl border border-gray-200 bg-gradient-to-br from-gray-50 to-white p-8 sm:p-10 shadow-sm text-center">
            ...
            <div class="mt-8 flex flex-col sm:flex-row flex-wrap items-center justify-center gap-3">
                <a href="{{ $landingUrl }}"
                   ...
                   data-track="blog_tuxtla_web_final_landing">
                    {{ $bp['final_cta_landing'] }}
                </a>
                <a href="{{ route('website_quote') }}"
                   class="inline-flex justify-center items-center w-full sm:w-auto min-w-48 px-8 py-3.5 rounded-full border-2 border-[#2C2C2C] font-semibold hover:bg-gray-50 transition-all"
                   data-track="blog_tuxtla_web_final_quote">
                    {{ $bp['final_cta_quote'] }}
                </a>
                <a href="{{ route('contact') }}"
                   class="inline-flex justify-center items-center w-full sm:w-auto min-w-48 px-8 py-3.5 rounded-full bg-gray-100 text-black font-semibold hover:bg-gray-200 transition-all"
                   data-track="blog_tuxtla_web_final_contact">
